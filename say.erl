...
say(#simple{id=ID,map=Map},Message,State = #state{maps=Maps}) ->
    ...
    userExists({ID,Map,MapDict,Message,Maps},dict:find(ID,MapDict),State).

userExists({ID,Map,MapDict,Message,Maps},{ok, Record},State) ->
    ...
    case Waited > ?FLOOD of 
        true ->
            es_websock:sendToAll(MapDict,User,["say @@@ ",User,"||",Message]);
        false ->
    ...
    case Test<1000 of
        false ->
            NewDict=dict:store(ID,Record#user{floodTest=FloodTest1,lastMessage=Unix,lastAction=Unix}),
            NewMap=array:set(Map,NewDict,Maps),
            State#state{maps=NewMap};
        true ->
